...
export function AgentStatus() {
  ...
  const [pollerStatus, setPollerStatus] = useState({ status: 'unknown', lastCycle: null });
  ...
  useEffect(() => {
    const fetchPollerStatus = async () => {
      try {
        ...
        setPollerStatus({ status: 'down', lastCycle: null });
      }
    };

    fetchPollerStatus();
    const interval = setInterval(fetchPollerStatus, 30000);
  ...
  return (
    <Panel title="Agents" loading={loading} error={error} className="h-full" flexContent>
      <div className="flex-1 overflow-y-auto min-h-0 space-y-2">
        ...
        {/* Spawner (Poller) Status */}
        <div className="border-t border-mission-border/30 mt-2 pt-2">
          <div className="flex items-center gap-3 px-2 py-1.5 rounded hover:bg-mission-bg/30">
            {/* Status Light */}
            <div className="relative flex-shrink-0">
