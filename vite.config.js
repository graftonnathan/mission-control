...
function getAgentStatus(agentId) {
  ...
  // First check workspace root for working file (legacy)
  ...
  if (fs.existsSync(rootWorkingFile)) {
    ...
    // NEW: Read phase from EXCHANGE/projects/
    const exchangeProjectDir = path.join(WORKSPACE_ROOT, 'EXCHANGE', 'projects', project);
    const phaseFile = path.join(exchangeProjectDir, 'phase');
    const legacyPhaseFile = path.join(projectsDir, project, '04-phase');
    const phase = readFile(phaseFile)?.trim() || readFile(legacyPhaseFile)?.trim() || 'unknown';
    return {
      status: 'working',
      project: project,
      phase: phase,
  ...
  if (fs.existsSync(exchangeWorkingFile)) {
    ...
    if (projectMatch) {
      ...
      // If still unknown, check for projects in fix/implement phase (NEW: use EXCHANGE/projects/)
      if (project === 'unknown') {
        const entries = readDir(projectsDir);
        for (const entry of entries) {
          if (entry.isDirectory()) {
            const exchangeProjectDir = path.join(WORKSPACE_ROOT, 'EXCHANGE', 'projects', entry.name);
            const phaseFile = path.join(exchangeProjectDir, 'phase');
            const legacyPhaseFile = path.join(projectsDir, entry.name, '04-phase');
            const projectPhase = readFile(phaseFile)?.trim() || readFile(legacyPhaseFile)?.trim();
    ...
    // Get phase if not already set (NEW: use EXCHANGE/projects/)
    if (phase === 'unknown' && project !== 'unknown') {
      const exchangeProjectDir = path.join(WORKSPACE_ROOT, 'EXCHANGE', 'projects', project);
      const phaseFile = path.join(exchangeProjectDir, 'phase');
      const legacyPhaseFile = path.join(projectsDir, project, '04-phase');
      phase = readFile(phaseFile)?.trim() || readFile(legacyPhaseFile)?.trim() || 'unknown';
    }
  ...
  // Check ticket system - NEW: scan tickets/active/ for .working files
  const ticketsDir = path.join(WORKSPACE_ROOT, 'EXCHANGE', 'tickets', 'active');
  if (fs.existsSync(ticketsDir)) {
    const ticketDirs = readDir(ticketsDir).filter(e => e.isDirectory());
    for (const ticketDir of ticketDirs) {
      const ticketPath = path.join(ticketsDir, ticketDir.name);
      const workingFile = path.join(ticketPath, '.working');
      
      if (fs.existsSync(workingFile)) {
        // Read ticket.json to get assignee and project
        const ticketJsonPath = path.join(ticketPath, 'ticket.json');
        if (fs.existsSync(ticketJsonPath)) {
          try {
            const ticketData = JSON.parse(fs.readFileSync(ticketJsonPath, 'utf-8'));
            if (ticketData.assignee?.toLowerCase() === agentIdLower) {
              return {
                status: 'working',
                project: ticketData.metadata?.project || ticketData.project || ticketDir.name,
                phase: ticketData.phase || 'unknown',
                task: ticketData.title || `${ticketData.phase} on ${ticketDir.name}`
              };
            }
          } catch (e) {
  ...
  // Fallback: check each project folder for working file (NEW: use EXCHANGE/projects/)
  const entries = readDir(projectsDir);
  for (const entry of entries) {
    if (entry.isDirectory()) {
  ...
  return {
    ...
    configureServer(server) {
      ...
      server.middlewares.use('/api/projects', (req, res, next) => {
        ...
        // NEW: Code in PROJECTS/, metadata in EXCHANGE/projects/
        const projectsDir = path.join(WORKSPACE_ROOT, 'PROJECTS');
        const exchangeProjectsDir = path.join(WORKSPACE_ROOT, 'EXCHANGE', 'projects');
        const ticketsDir = path.join(WORKSPACE_ROOT, 'EXCHANGE', 'tickets', 'active');
        const archiveDir = path.join(WORKSPACE_ROOT, 'EXCHANGE', 'tickets', 'archive');
        
        // Build project phase map from tickets (source of truth)
        const projectPhases = new Map();
        
        // Read active tickets for phases
        if (fs.existsSync(ticketsDir)) {
          const ticketEntries = readDir(ticketsDir).filter(e => e.isDirectory());
          for (const entry of ticketEntries) {
            const ticketJsonPath = path.join(ticketsDir, entry.name, 'ticket.json');
            if (!fs.existsSync(ticketJsonPath)) continue;
            try {
              const ticketData = JSON.parse(fs.readFileSync(ticketJsonPath, 'utf-8'));
              const projectName = ticketData.metadata?.project || entry.name;
              projectPhases.set(projectName, {
                phase: ticketData.phase || 'unknown',
                priority: ticketData.metadata?.priority || 5,
                blocked: ticketData.phase === 'blocked',
                ticketId: entry.name
              });
            } catch (e) {
              console.error(`Failed to parse ticket ${entry.name}:`, e.message);
            }
          }
        }
        
        // Read archived tickets for completed projects
        if (fs.existsSync(archiveDir)) {
          const archiveEntries = readDir(archiveDir).filter(e => e.isDirectory());
          for (const entry of archiveEntries) {
            const ticketJsonPath = path.join(archiveDir, entry.name, 'ticket.json');
            if (!fs.existsSync(ticketJsonPath)) continue;
            try {
              const ticketData = JSON.parse(fs.readFileSync(ticketJsonPath, 'utf-8'));
              const projectName = ticketData.metadata?.project || entry.name;
              if (!projectPhases.has(projectName)) {
                projectPhases.set(projectName, {
                  phase: 'complete',
                  priority: ticketData.metadata?.priority || 5,
                  blocked: false,
              ...
              console.error(`Failed to parse archived ticket ${entry.name}:`, e.message);
            }
          }
        }
        
        // Read from PROJECTS directory (actual code location)
        let projects = [];
        if (fs.existsSync(projectsDir)) {
          ...
          projects = entries
            ...
            .map(e => {
              ...
              // Get metadata from EXCHANGE/projects/ or fallback to defaults
              const exchangeProjectPath = path.join(exchangeProjectsDir, e.name);
              const phaseFile = path.join(exchangeProjectPath, 'phase');
              const priorityFile = path.join(exchangeProjectPath, 'priority');
              const blockedFile = path.join(exchangeProjectPath, 'blocked');
              const tokensFile = path.join(exchangeProjectPath, 'tokens.json');
              
              // Phase from EXCHANGE/projects/, fallback to ticket, fallback to 'unknown'
              const ticketInfo = projectPhases.get(e.name);
              const exchangePhase = readFile(phaseFile)?.trim();
              const phase = exchangePhase || ticketInfo?.phase || 'unknown';
              
              const priority = parseInt(readFile(priorityFile)?.trim() || ticketInfo?.priority || '5', 10);
              const blocked = fs.existsSync(blockedFile) || ticketInfo?.blocked || false;
        ...
        // Add any ticket-only projects (no code yet)
        projectPhases.forEach((info, projectName) => {
        ...
        // Fallback: Read projects without tickets from PROJECTS/
        if (projects.length === 0) {
          ...
          projects = entries
            ...
            .map(e => {
              ...
              // NEW: Read from EXCHANGE/projects/{name}/
              const phaseFile = path.join(exchangeProjectPath, 'phase');
              const priorityFile = path.join(exchangeProjectPath, 'priority');
              const tokensFile = path.join(exchangeProjectPath, 'tokens.json');
              const blockedFile = path.join(exchangeProjectPath, 'blocked');
              ...
              const legacyPhaseFile = path.join(projectPath, '04-phase');
              const legacyPriorityFile = path.join(projectPath, '05-priority');
              const legacyTokensFile = path.join(projectPath, '09-tokens.json');
              const legacyBlockedFile = path.join(projectPath, '05-blocked');
              
              const phase = readFile(phaseFile)?.trim() || readFile(legacyPhaseFile)?.trim() || 'unknown';
              const priority = parseInt(readFile(priorityFile)?.trim() || readFile(legacyPriorityFile)?.trim() || '5', 10);
              const blocked = fs.existsSync(blockedFile) || fs.existsSync(legacyBlockedFile);
              const tokensData = readFile(tokensFile) || readFile(legacyTokensFile);
              const tokens = tokensData ? JSON.parse(tokensData) : null;
      ...
      // GET /api/tickets - list tickets from EXCHANGE/tickets/active and archive
      server.middlewares.use('/api/tickets', (req, res, next) => {
        if (req.method !== 'GET') return next();
        ...
        if (fs.existsSync(ticketsDir)) {
          ...
          const activeTickets = ticketDirs.map(e => {
            const ticketJsonPath = path.join(ticketsDir, e.name, 'ticket.json');
            const workingFile = path.join(ticketsDir, e.name, '.working');
            
            if (!fs.existsSync(ticketJsonPath)) return null;
            
            try {
              const content = fs.readFileSync(ticketJsonPath, 'utf-8');
              const data = JSON.parse(content);
              return {
                ...data,
                id: e.name,
                active: fs.existsSync(workingFile),
                archived: false,
                path: path.join(ticketsDir, e.name)
              };
            } catch (err) {
              console.error(`Failed to parse ticket ${e.name}:`, err.message);
              return null;
            }
          }).filter(Boolean);
          
          tickets = tickets.concat(activeTickets);
        }
        
        // Read archived tickets (completed) - show last 10
        if (fs.existsSync(archiveDir)) {
          const archiveDirs = readDir(archiveDir)
            .filter(e => e.isDirectory())
            .sort((a, b) => {
              // Sort by mtime descending
              const statA = fs.statSync(path.join(archiveDir, a.name, 'ticket.json'));
              const statB = fs.statSync(path.join(archiveDir, b.name, 'ticket.json'));
              return statB.mtime - statA.mtime;
            })
            .slice(0, 10); // Only last 10 completed
          
          const archivedTickets = archiveDirs.map(e => {
            const ticketJsonPath = path.join(archiveDir, e.name, 'ticket.json');
            ...
            try {
              ...
              console.error(`Failed to parse archived ticket ${e.name}:`, err.message);
              return null;
            }
          }).filter(Boolean);
      ...
      // GET /api/poller-status - get poller health status
      server.middlewares.use('/api/poller-status', (req, res, next) => {
        if (req.method !== 'GET') return next();
        
        const statusFile = path.join(WORKSPACE_ROOT, 'EXCHANGE', 'poller-status.json');
        ...
        if (fs.existsSync(statusFile)) {
          try {
            ...
            const lastCycle = new Date(data.lastCycle);
            const now = new Date();
            const minutesSinceCycle = (now - lastCycle) / 1000 / 60;
            
            if (minutesSinceCycle < 10) {
              status.status = 'running';
              status.message = 'Poller is healthy';
            } else if (minutesSinceCycle < 30) {
              status.status = 'stalled';
              status.message = `Last cycle ${Math.round(minutesSinceCycle)} min ago`;
            } else {
              status.status = 'down';
              status.message = `Last cycle ${Math.round(minutesSinceCycle)} min ago`;
            }
            
            status.lastCycle = data.lastCycle;
            status.activeTickets = data.activeTickets || 0;
          } catch (e) {
            status.status = 'error';
            status.message = 'Failed to parse status file';
          }
        } else {
          status.status = 'down';
        }
      ...
      server.middlewares.use('/api/reports', (req, res, next) => {
        ...
        // Read from EXCHANGE/tickets/archive (completed tickets)
        const archiveDir = path.join(WORKSPACE_ROOT, 'EXCHANGE', 'tickets', 'archive');
        if (fs.existsSync(archiveDir)) {
          const archiveEntries = readDir(archiveDir).filter(e => e.isDirectory());
          const archivedTickets = archiveEntries.map(e => {
            const ticketJsonPath = path.join(archiveDir, e.name, 'ticket.json');
            if (!fs.existsSync(ticketJsonPath)) return null;
            ...
            try {
              const content = fs.readFileSync(ticketJsonPath, 'utf-8');
              const data = JSON.parse(content);
              const stats = fs.statSync(ticketJsonPath);
              
              // Get last history entry to find who completed it
              const lastEntry = data.history?.[data.history.length - 1];
              const agent = lastEntry?.agent || 'system';
              
              // Build report content from ticket
              const reportContent = `# Completed: ${data.title}\n\n` +
                `**Ticket:** ${data.id}\n` +
                `**Project:** ${data.metadata?.project || 'unknown'}\n` +
                `**Completed:** ${lastEntry?.time || stats.mtime}\n` +
                `**By:** ${agent}\n\n` +
                `## Summary\n${data.description || 'No description'}\n\n` +
                `## History\n${data.history?.map(h => `- ${h.phase}: ${h.agent} - ${h.summary || 'no summary'}`).join('\n') || 'No history'}\n`;
          ...
          allReports = allReports.concat(archivedTickets);
        }
      ...
      server.middlewares.use('/api/projects', (req, res, next) => {
        ...
        try {
          ...
          if (fs.existsSync(pm2Config)) {
            ...
            checkCommand = `lsof -i :8787 2>/dev/null | grep LISTEN && echo "running" || echo "stopped"`;
          ...
          exec(checkCommand, (error, stdout, stderr) => {
            ...
            if (checkType === 'pm2') {
              ...
              // For mission-control and Kinectv1, stdout will include "running" or "stopped"
              if (projectName === 'mission-control' || projectName === 'Kinectv1') {
      ...
      server.middlewares.use('/api/projects', (req, res, next) => {
        ...
        req.on('end', () => {
          try {
            ...
            const projectDir = path.join(WORKSPACE_ROOT, 'PROJECTS', projectName);
            const exchangeProjectDir = path.join(WORKSPACE_ROOT, 'EXCHANGE', 'projects', projectName);
            const phaseFile = path.join(exchangeProjectDir, 'phase');
            ...
            // Ensure EXCHANGE/projects/{name}/ directory exists
            if (!fs.existsSync(exchangeProjectDir)) {
              fs.mkdirSync(exchangeProjectDir, { recursive: true });
            }
      ...
      server.middlewares.use('/api/projects', (req, res, next) => {
        ...
        req.on('end', () => {
          try {
            ...
            const projectDir = path.join(WORKSPACE_ROOT, 'PROJECTS', projectName);
            const exchangeProjectDir = path.join(WORKSPACE_ROOT, 'EXCHANGE', 'projects', projectName);
            const blockedFile = path.join(exchangeProjectDir, 'blocked');
      ...
      server.middlewares.use('/api/architect/prompt', (req, res, next) => {
        ...
        req.on('end', () => {
          try {
            ...
            // Create project directory structure (NEW: ticket-based system)
            const projectDir = path.join(WORKSPACE_ROOT, 'PROJECTS', projectName);
            const exchangeProjectDir = path.join(WORKSPACE_ROOT, 'EXCHANGE', 'projects', projectName);
            const ticketsDir = path.join(WORKSPACE_ROOT, 'EXCHANGE', 'tickets', 'active');
            ...
            // 1. Create PROJECTS/{name}/ directory (code will go here)
            fs.mkdirSync(projectDir, { recursive: true });
            fs.writeFileSync(path.join(projectDir, 'README.md'), `# ${projectName}\n\n${prompt}\n`);

            // 2. Create EXCHANGE/projects/{name}/ directory (metadata)
            fs.mkdirSync(exchangeProjectDir, { recursive: true });
            fs.writeFileSync(path.join(exchangeProjectDir, 'phase'), 'architecture');
            fs.writeFileSync(path.join(exchangeProjectDir, 'priority'), '5');
            fs.writeFileSync(path.join(exchangeProjectDir, 'prompt.md'), prompt);

            // 3. Create TICKET in EXCHANGE/tickets/active/ (workflow)
            fs.mkdirSync(ticketsDir, { recursive: true });
            const ticketId = `TICKET-${Date.now()}-${projectName.toLowerCase().replace(/\s+/g, '-')}`;
            ...
            fs.writeFileSync(path.join(ticketDir, 'ticket.json'), JSON.stringify({
              ...
              history: [{
                ...
                summary: `Created new project "${projectName}" from Mission Control`
              }],
              deliverables: {
                plan: '01-plan.md',
            ...
            // Create 01-plan.md in ticket directory
            fs.writeFileSync(path.join(ticketDir, '01-plan.md'), `# Plan: ${projectName}\n\n${prompt}\n`);
